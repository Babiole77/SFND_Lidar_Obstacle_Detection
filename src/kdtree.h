...
struct KdTree
{
	...
	{
		std::vector<int> ids;
		ids = searchHelper(target, root, 0, distanceTol, ids);
		
		return ids;
	}
	std::vector<int> searchHelper(std::vector<float> target, Node* node, uint depth, float distanceTol, std::vector<int>& ids)
	{
		if(node!=NULL)
		{
